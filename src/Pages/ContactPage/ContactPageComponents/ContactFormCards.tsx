  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full lg:grid-cols-1 lg:w-[30%] space-y-3 text-center">
        {formContext.map((el, index) => (
          <div
            key={index}
            className="px-3 py-8 bg-[#FCFCFD] border border-[#F1F1F3] flex flex-col items-center justify-center space-y-5 rounded-lg"
          >
            <div className="flex space-x-3">
              {el.Icon.map((el, index) => (
                <a
                  key={index}
                  href={el.link}
